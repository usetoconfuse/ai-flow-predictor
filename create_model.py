...
model_name = "firstimproved_1000epochs"
...
for nodes_per_layer in range(math.floor(input_size/2), input_size*2):
    ...
    error_vals = trn.epoch_error_calcs(raw_trained_val_predictions[-1], raw_predictands)
    ...
    if performance < best_performance:
        ...
        destd_trained_trn_predictions = dp.destd_array(trn_predictions, std_metadata.iloc[:, -1])
        raw_trained_trn_predictions = np.exp(destd_trained_trn_predictions)
...
dp.write_model_json(best_network, best_trn_predictions.tolist(), best_val_predictions.tolist(), model_name)